              ...
              <tr>
                <td style="padding: 20px;">
                  ...
                  <p style="margin: 0 0 8px 0; font-size: 12px; color: #6b7280; font-weight: 500;">
                    ${escapeHtml(article.sourceName)} · ${article.publishedAt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </p>

                  <!-- Title -->
                  <h3 style="margin: 0 0 12px 0; font-size: 18px; line-height: 1.4;">
                    <a href="${escapeUrl(article.url)}" style="color: #111827; text-decoration: none;">${escapeHtml(article.title)}</a>
                  </h3>

                  <!-- Briefing -->
                  ${article.briefing ? `
                  <p style="margin: 0 0 16px 0; color: #374151; font-size: 15px; line-height: 1.7;">
                    ${escapeHtml(article.briefing)}
                  </p>
                  ` : (article.summary ? `
                  <p style="margin: 0 0 16px 0; color: #374151; font-size: 15px; line-height: 1.7;">
                  ...
                  <p style="margin: 0 0 16px 0;">
                    ${article.tags.map((tag) => `<span style="display: inline-block; background: #f3f4f6; color: #4b5563; padding: 4px 10px; border-radius: 100px; font-size: 12px; margin-right: 6px; margin-bottom: 6px;">${escapeHtml(tag)}</span>`).join('')}
                  </p>
                  ` : ''}
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border-radius: 8px; margin-top: 4px;">
                    <tr>
                      <td style="padding: 14px 16px;">
                        ...
                        ${article.contentAngles.map((angle) => `<p style="margin: 0 0 4px 0; font-size: 13px; color: #78350f; padding-left: 12px;">→ ${escapeHtml(angle)}</p>`).join('')}
                      </td>
                    </tr>
                  </table>
                  ` : ''}
                  ...
                  <table cellpadding="0" cellspacing="0" border="0" style="margin-top: 16px;">
                    <tr>
                      <td style="background: #111827; border-radius: 6px;">
                        <a href="${escapeUrl(article.url)}" style="display: inline-block; padding: 10px 20px; color: #ffffff; text-decoration: none; font-size: 13px; font-weight: 500;">Read Article →</a>
                      </td>
                    </tr>
                  </table>
                </td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border-radius: 12px; border-left: 4px solid #3b82f6;">
                <tr>
                  <td style="padding: 24px;">
                    ...
                    <p style="margin: 0; color: #1e3a5f; font-size: 15px; line-height: 1.8;">${escapeHtml(digestIntro)}</p>
                  </td>
                </tr>
              </table>
            </td>
